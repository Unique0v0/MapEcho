...
class MapTransformerDecoder_new(BaseModule):
    ...
    def __init__(self, 
        ...
        self.last_prop_mask = None
        self.last_prop_valid_idx = None
        self.last_prop_add_lid = None
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            if lid == self.prop_add_stage and prop_query is not None and prop_reference_points is not None:
                ...
                valid_idx_list = []
                for i in range(bs):
                    prop_mask = output.new_zeros((num_queries, ), dtype=torch.bool)
                    if is_first_frame_list[i]:
                        ...
                        prop_mask[:topk] = True
                        valid_idx_list.append(valid_idx.detach())
                    prop_masks.append(prop_mask)
                
                output = torch.stack(new_query).permute(1, 0, 2)
                reference_points = torch.stack(new_refpts)
                self.last_prop_mask = torch.stack(prop_masks)
                self.last_prop_valid_idx = valid_idx_list
                self.last_prop_add_lid = lid

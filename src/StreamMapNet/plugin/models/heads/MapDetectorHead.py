...
class MapDetectorHead(nn.Module):
    ...
    def _debug_enabled(self, name):
        if not self.debug_cfg.get('enabled', False):
            return False
        return self.debug_cfg.get(name, True)
    ...
    def _debug_frame_path(self, category, img_meta):
        out_dir = self.debug_cfg.get('out_dir', os.path.join('debug', category))
    ...
    def _dump_debug(self, category, img_meta, payload):
        if not self._debug_enabled(category):
            return
        torch.save(self._to_cpu_debug(payload), self._debug_frame_path(category, img_meta))

    def reset_streaming_query(self):
        if not self.streaming_query:
            return
        self.query_memory.reset_all()
        self.reference_points_memory.reset_all()
        self.target_memory.reset_all()

    def _dump_query_memory_update(self, img_meta, is_first_frame, scores, queries,
                                  lines, topk_idx, prop_mask=None):
        if not self._debug_enabled('query_memory'):
            return

        score_raw, labels = scores.max(-1)
        payload = {
            'scene_name': img_meta.get('scene_name'),
            'frame_idx': img_meta.get('sample_idx'),
            'token': img_meta.get('token'),
            'is_first_frame': is_first_frame,
            'topk_idx': topk_idx,
            'topk_score_raw_logit': score_raw[topk_idx],
            'topk_score_sigmoid': score_raw[topk_idx].sigmoid(),
            'topk_label': labels[topk_idx],
            'topk_query_embedding': queries[topk_idx],
            'topk_reference_points': lines[topk_idx].view(-1, self.num_points, 2),
            'topk_pred_vectors': lines[topk_idx].view(-1, self.num_points, 2),
            'all_query_scores_raw_logit': scores,
            'all_query_scores_sigmoid': scores.sigmoid(),
            'all_query_labels': labels,
            'all_query_pred_vectors': lines.view(-1, self.num_points, 2),
            'propagated_query_mask': prop_mask,
        }
        self._dump_debug('query_memory', img_meta, payload)
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        if self.streaming_query:
            ...
            decoder_prop_mask = getattr(self.transformer.decoder, 'last_prop_mask', None)

            for i in range(bs):
                _lines = lines[i]
                _queries = inter_queries[-1][i]
                _raw_scores = scores[i]
                _gt_targets = gt_lines[i] # (num_q or num_q+topk, 20, 2)
                assert len(_lines) == len(_queries)
                assert len(_lines) == len(_gt_targets)

                _scores, _ = _raw_scores.max(-1)
                topk_score, topk_idx = _scores.topk(k=self.topk_query, dim=-1)
                prop_mask = decoder_prop_mask[i] if decoder_prop_mask is not None else None
                self._dump_query_memory_update(
                    img_metas[i], is_first_frame_list[i], _raw_scores,
                    _queries, _lines, topk_idx, prop_mask)
    ...
    def forward_test(self, bev_features, img_metas):
        ...
        outputs = []
        decoder_prop_mask = getattr(self.transformer.decoder, 'last_prop_mask', None)
        prop_add_stage = getattr(self.transformer.decoder, 'prop_add_stage', 0)
        for layer_idx, (queries) in enumerate(inter_queries):
            ...
            for i in range(len(scores)):
                ...
                reg_points_list.append(reg_points[i])
                scores_list.append(scores[i])
                if (decoder_prop_mask is not None and
                        layer_idx >= prop_add_stage):
                    prop_mask = decoder_prop_mask[i].to(scores.device)
                else:
                    prop_mask = scores.new_zeros((len(scores[i]), ), dtype=torch.bool)

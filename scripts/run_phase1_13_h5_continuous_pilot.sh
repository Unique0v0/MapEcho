...
OUT_ROOT=${OUT_ROOT:-/data/dj/MapEcho/artifacts/phase1_8b_downstream/phase1_13_h5_continuous_pilot/n_attack3_controlled_check}
...
RECOVERY=${RECOVERY:-9}
N_ATTACK=${N_ATTACK:-3}
ATTACK_START_OFFSET=${ATTACK_START_OFFSET:-0}
RESET_AFTER_OFFSET=${RESET_AFTER_OFFSET:-$((ATTACK_START_OFFSET + N_ATTACK - 1))}
...
while IFS= read -r TOKEN; do
  ...
  if [[ "${SKIP_COMPLETED}" == "1" ]] \
    ...
    echo "[MapEcho] skipping completed H5 token ${TOKEN}"
    continue
  fi

  echo "[MapEcho] building clean sequence ann for H5 ${TOKEN}"
  ...
  echo "[MapEcho] building N=${N_ATTACK} multi-frame ann for ${TOKEN}"
  /home/dj/.conda/envs/maptr/bin/python scripts/build_multi_frame_glare_sequence_ann.py \
    --clean-ann "${CLEAN_ANN}" \
    --asset-csv "${ASSET_CSV}" \
    --out-ann "${ATTACK_ANN}" \
    --out-dir "${ROOT}/multi_frame_assets" \
    --attack-objective eta \
    --source-frame lidar \
    --attack-start-offset "${ATTACK_START_OFFSET}" \
    --num-attack-frames "${N_ATTACK}" \
    --power "${ATTACK_POWER}" \
  ...
  /home/dj/.conda/envs/maptr4090/bin/python /home/dj/MapEcho/scripts/run_streammapnet_multi_condition.py \
    ...
    --reset-after-offset "${RESET_AFTER_OFFSET}" \
    --skip-completed
